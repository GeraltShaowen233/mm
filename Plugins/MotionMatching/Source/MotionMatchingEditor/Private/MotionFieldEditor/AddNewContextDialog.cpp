...
bool SAddNewContextDialog::FilterAnim(const FAssetData & AssetData)
{
	...
	auto Ass = Cast<UAnimSequence>(AssetData.GetAsset())->GetSkeleton();
	bool sameSkeleton = (SkeletonName != Cast<UAnimSequence>(AssetData.GetAsset())->GetSkeleton()->GetName());
	return AlreadyAdded || sameSkeleton;
}
